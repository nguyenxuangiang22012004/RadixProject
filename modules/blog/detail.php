...
if (!empty(getBody('get')['id'])){
    ...
    $blogDetail = firstRaw("SELECT blog.*, blog_categories.name as cate_name, blog_categories.id as cate_id, users.fullname, users.email, groups.name as group_name, users.about_content, users.contact_facebook, users.contact_twitter, users.contact_linkedin, users.contact_pinterest, (SELECT count(id) FROM blog WHERE user_id=users.id) as blog_count FROM blog INNER JOIN blog_categories ON blog.category_id=blog_categories.id INNER JOIN users ON blog.user_id=users.id INNER JOIN `groups` ON users.group_id=`groups`.id WHERE blog.id=$id");
                                ...
                                <div class="blog-inner">
                                    <div class="blog-top">
                                        ...
                                        <ul class="social-share">
                                            ...
                                            <li><a href="https://www.linkedin.com/cws/share?url=<?php echo _WEB_HOST_ROOT.'?module=blog&action=detail&id='.$id; ?>" target="_blank"><i class="fa fa-linkedin"></i></a></li>
                                    ...
                                    <?php
                                    echo html_entity_decode($blogDetail['content']);
                                    ?>
