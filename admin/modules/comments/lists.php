...
//3. Tính số trang
$maxPage = ceil($allCommentsNum/$perPage); //Làm tròn lên
        ...
        <div class="container-fluid">
            ...
            <form action="" method="get">
                <div class="row">
                    ...
                    <div class="col-4">
                        <input type="search" name="keyword" class="form-control" placeholder="Từ khoá tìm kiếm..." value="<?php echo (!empty($keyword))?$keyword:false; ?>"/>
                    </div>
                    <div class="col-2">
                ...
                if (!empty($listComments)):
                    foreach ($listComments as $key => $item):
                        ...
                        <tr>
                            ...
                            <?php
                            if (!empty($item['webiste'])){
                                echo '- Website: '.$item['webiste'].'<br/>';
                            }
                            ?>
                            ...
                            <td>
                                <?php
                                echo ($item['status']==0)?'<button class="btn btn-danger btn-sm">Chưa duyệt</button>':'<button class="btn btn-success btn-sm">Đã duyệt</button> <br/>';

                                if ($item['status']==0){
                                    echo '<a href="'._WEB_HOST_ROOT_ADMIN.'/?module=comments&action=status&id='.$item['id'].'&status=1">Duyệt</a>';
                                }else{
